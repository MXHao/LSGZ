...
Page({
  ...
  popuBuy(e) {
    console.log(e)
    this.setData({
      popuPro: e.currentTarget.dataset.item,
      flag3: true
    })
    this.countPrice();
  },
  selectPopuProductInfo() {
    app.api.request("selectPopuProductInfo", {
      clientId: this.data.clientId,
      price: this.data.sumPrice
    }, res => {
      this.setData({
        popus: res.data.data.products,
        popuInfo: res.data.data.popuInfo
      })
    })
  },
  init() {
  ...
  countPrice(I) {
    ...
    if (typeof(this.data.discountCount) == "undefined") {
      this.setData({
        discountCount: 0.01
      })
    }
    ...
    if (this.data['popuInfo'] && this.data['popuInfo']['isDisCount'] == '1') {
      sum = sum * (parseFloat(this.data['popuInfo']['discount']) / 100)
    }
    if (sum <= 0.01) {
      sum = 0.01;
    }
    ...
    if (app.user.openid) {
      ...
      if (this.data.pageType) {
          ...
          Object.assign(res.data.data, {
            ...
            'complete': function(res) {
              if (res.errMsg.indexOf("ok") != -1) {
                ...
                wx.redirectTo({
                  url: '/pages/PaymentResult/PaymentResult?price=' + that.data.sumPrice + "&id=" + tmpRes.orderNo + "&terminal=" + (that.data.pageType ? 1 : 2)
                })
              } else {
                wx.redirectTo({
        ...
        app.api.request('createOrder_api', {
          ...
          activityId: this.data.group && this.data.group == 2 ? this.data.aId : "",
          price: this.data.sumPrice,
          popu: this.data.popuPro ? { ...this.data.popuInfo,
            ...this.data.popuPro
          } : null,
          plus: this.data.selectPlusIndex != "" ? this.data.sysParam.plusMoney[this.data.selectPlusIndex] : ""
        }, res => {
